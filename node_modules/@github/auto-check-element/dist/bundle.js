...
(function(AllowedHttpMethods2) {
  AllowedHttpMethods2["GET"] = "GET";
  AllowedHttpMethods2["POST"] = "POST";
...
var AutoCheckElement = class extends HTMLElement {
  ...
  set validateOnKeystroke(enabled) {
    if (enabled) {
...
function handleChange(checker, event) {
  ...
  if (input.value.length === 0)
    return;
  if (event.type !== "blur" && !autoCheckElement.onlyValidateOnBlur || event.type === "blur" && autoCheckElement.onlyValidateOnBlur && !autoCheckElement.validateOnKeystroke && autoCheckElement.hasAttribute("dirty") || event.type === "input" && autoCheckElement.onlyValidateOnBlur && autoCheckElement.validateOnKeystroke || event.type === "triggervalidation") {
